...
class OFAEncoder(OFAPreTrainedModel):
    ...
    def __init__(self, config: OFAConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        if config.add_type_embedding:
            ...
            self.type_embedding_2 = Embedding(1, self.embed_dim, padding_idx=None)
        else:
    ...
    def forward_embedding(
        ...
        if image_embed_2 is not None:
            ...
            if self.type_embedding_2 is not None:
                image_x_2 += self.type_embedding_2(input_ids.new_zeros(image_x_2.size()[:2]))
            if self.patch_layernorm_embedding is not None:
    ...
    def forward(
        ...
        if patch_images_2 is not None:
            ...
            # image_padding_mask_2[~patch_masks] = True # comment the line to temporarily fix the bug of mismatch
